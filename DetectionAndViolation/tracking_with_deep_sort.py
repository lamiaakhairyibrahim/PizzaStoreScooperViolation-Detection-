    ...
    def frame_detect(self):
        ...
        def callback(ch, method, properties, body):
            ...
            for box in results.boxes:
                ...
                if label in ["hand", "scooper", "pizza"]:
            ...
            for track in tracks:
                ...
                box_coords = (int(l), int(t), int(w), int(h))

                color = (0, 255, 0)
                if label == "hand":
                    tracked_hand.append((track_id, box_coords))
                    if track_id in violations:
                        color = (0, 0, 255)
                elif label == "scooper":
                    tracked_scooper.append((track_id, box_coords))
                    color = (255, 255, 0)
                elif label == "pizza":
                    tracked_pizza.append((track_id, box_coords))
                    color = (255, 100, 180)

                cv2.rectangle(frame, (int(l), int(t)), (int(l + w), int(t + h)), color, 2)
                cv2.putText(frame, f"{label}-{track_id}", (int(l), int(t) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
        ...
        print(" Detection service is running with DeepSORT...")
